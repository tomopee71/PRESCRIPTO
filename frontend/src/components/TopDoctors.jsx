...
const TopDoctors = () => {
  ...
  return (
    <div className="flex flex-col items-center gap-4 my-16 text-gray-900 md:mx-10">
      ...
      <p className="sm:w-1/3 text-center text-sm">信頼できるスタッフを紹介します。</p>
          ...
          <div onClick={() => navigate(`/appointment/${item._id}`)} className="border border-green-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500" key={index}>
            ...
            <div className="p-4">
              <div className="flex items-center gap-2 text-sm text-center text-green-500">
                <p className="w-2 h-2 bg-green-500 rounded-full"></p><p>予約可能</p>
              </div>
      ...
      <button onClick={() => { navigate('/doctors'); window.scrollTo(0, 0); }} className="bg-green-100 text-gray-600 px-12 py-3 rounded-full mt-10">もっと見る</button>
    </div>
  );
};
